 ...
 * 
 * Now includes SPADE (Spatially-Adaptive Denormalization) hierarchical
 * conditioning with trainable γ/β parameters at Level 1 (COARSE).
 */
...
class TemporalConditioning {
    constructor(visualizer) {
        ...
        this.config = {
            mode: 'smoothstep',  // step, linear, smoothstep, custom, spade
            transitionZone: 10,  // frames
            skeletonAware: true, // use skeleton-aware blending
            spadeLevel: 1        // SPADE hierarchy level (1=COARSE optimal)
        };
    }
    ...
    setupEventListeners() {
        ...
        if (blendModeSelect) {
            blendModeSelect.addEventListener('change', (e) => {
                ...
                // Log SPADE selection
                if (e.target.value === 'spade') {
                    console.log('🔷 SPADE mode enabled - using hierarchical conditioning at Level', this.config.spadeLevel || 1);
                }
            });
        }
        
        // SPADE level selector
        const spadeLevelSelect = document.getElementById('spadeLevel');
        if (spadeLevelSelect) {
            spadeLevelSelect.addEventListener('change', (e) => {
                this.config.spadeLevel = parseInt(e.target.value);
                console.log(`🔷 SPADE level changed to Level ${this.config.spadeLevel}`);
                this.updateModeDescription();
                this.updateBlendCurveVisualization();
            });
        }
    ...
    updateModeDescription() {
        const descriptions = {
            ...
            spade: '🔷 SPADE hierarchical conditioning with trainable γ/β at Level 1 (COARSE) - optimal FID/coverage'
        };
        ...
        // Show/hide SPADE level controls
        const spadeLevelControls = document.getElementById('spadeLevelControls');
        if (spadeLevelControls) {
            spadeLevelControls.style.display = this.config.mode === 'spade' ? 'block' : 'none';
        }
        ...
        if (indicator) {
            const modeName = this.config.mode === 'spade' 
                ? 'SPADE L' + (this.config.spadeLevel || 1)
                : this.config.mode.charAt(0).toUpperCase() + this.config.mode.slice(1);
    ...
    calculateTemporalWeight(t, boundary, transitionZone) {
        ...
        switch (mode) {
            ...
            case 'spade':
                // SPADE uses sigmoid for transition with trainable γ/β modulation
                // The actual γ/β modulation happens server-side in PyTorch
                // Here we use a sigmoid-like curve for visualization
                // σ(x) where x maps normalizedT from [-5, 5] for sharp transition
                const x = (normalizedT - 0.5) * 10;  // Map to [-5, 5]
